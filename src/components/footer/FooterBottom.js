      ...
      <div className="max-w-5xl mx-auto px-4">
        <div className="w-full grid grid-cols-3 md:grid-cols-5 mdl:grid-cols-6 lgl:grid-cols-7 gap-3 place-content-center text-gray-400">
          {footerBottomItem.map((item, index) => (
            <div className="group cursor-pointer" key={item._id || index}>
              <h3 className="w-24 font-semibold text-[12px]  group-hover: underline text-[#DDD] leading-3 mb[2px]">
                {item.title}
              </h3>
              <p className="w-24 mt-1 tracking-tight text-[12px] text-[#999] hover:text-gray-100 group-hover: underline leading-3">
                {item.des}
              </p>
            </div>
          ))}
